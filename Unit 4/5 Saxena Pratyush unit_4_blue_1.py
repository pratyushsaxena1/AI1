...
def grade_board(board, token):
    ...
    subboard_winners_list = [get_subboard_winner(board[key]) for key in board]
    # Get subboard wins
    ...
    for i in range(3):
        if subboard_winners_list[i * 3] == subboard_winners_list[i * 3 + 1] == token and subboard_winners_list[i * 3 + 2] is None:
            score += 200
        if subboard_winners_list[i] == subboard_winners_list[i + 3] == token and subboard_winners_list[i + 6] is None:
            score += 200
    # Two in a row with third occupied by enemy
    for i in range(3):
        if subboard_winners_list[i * 3] == subboard_winners_list[i * 3 + 1] == token and subboard_winners_list[i * 3 + 2] == enemy_token:
            score -= 150
        if subboard_winners_list[i] == subboard_winners_list[i + 3] == token and subboard_winners_list[i + 6] == enemy_token:
            score -= 150
    # Block three in a row for enemy
    for i in range(3):
        if subboard_winners_list[i * 3] == subboard_winners_list[i * 3 + 1] == enemy_token and subboard_winners_list[i * 3 + 2] == token:
            score += 150
        if subboard_winners_list[i] == subboard_winners_list[i + 3] == enemy_token and subboard_winners_list[i + 6] == token:
            score += 150
    ...
    for k in board:
        ...
        # Two in a row with third empty
        for i in range(3):
            if temp_board[i * 3] == temp_board[i * 3 + 1] == token and temp_board[i * 3 + 2] == '.':
                score += 5
            if temp_board[i] == temp_board[i + 3] == token and temp_board[i + 6] == '.':
                score += 5
        # Two in a row with third occupied by enemy
        for i in range(3):
            if temp_board[i * 3] == temp_board[i * 3 + 1] == token and temp_board[i * 3 + 2] == enemy_token:
                score -= 5
            if temp_board[i] == temp_board[i + 3] == token and temp_board[i + 6] == enemy_token:
                score -= 5
        # Block three in a row for enemy
        for i in range(3):
            if temp_board[i * 3] == temp_board[i * 3 + 1] == enemy_token and temp_board[i * 3 + 2] == token:
                score += 20
            if temp_board[i] == temp_board[i + 3] == enemy_token and temp_board[i + 6] == token:
                score += 20
