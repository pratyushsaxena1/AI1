...
def test_solution(puzzle):
    return all(i == '.' or len(i) == 1 for i in puzzle)

def get_constraints(puzzle):
    current_N, current_subblock_width, current_subblock_height, _ = get_puzzle_information(puzzle)
    rows = [set(range(i * current_N, (i + 1) * current_N)) for i in range(current_N)]
    columns = [set(range(i, current_N ** 2, current_N)) for i in range(current_N)]
    blocks = []
    for outer_loop_tracker_1 in range(0, current_N, current_subblock_width):
        for outer_loop_tracker_2 in range(0, current_N, current_subblock_height):
            block = set()
            for inner_loop_tracker_1 in range(current_subblock_width):
                for inner_loop_tracker_2 in range(current_subblock_height):
                    block.add((current_N * (outer_loop_tracker_2 + inner_loop_tracker_2)) + (outer_loop_tracker_1 + inner_loop_tracker_1))
            blocks.append(block)
    return rows, columns, blocks

def get_neighbors(puzzle):
    rows, columns, blocks = get_constraints(puzzle)
    neighbors_dict, index_tracker = {}, 0
    for _ in puzzle:
        neighbors = next((row for row in rows if index_tracker in row), set()).union(next((column for column in columns if index_tracker in column), set()).union(next((block for block in blocks if index_tracker in block), set())))
...
def get_most_constrained_variable(puzzle):
    ...
    for index, value in enumerate(puzzle):
        current_possibilities = len(value)
        if 1 < current_possibilities < min_possibilities:
            min_possibilities, most_constrained_variable = current_possibilities, index
...
with open(file_name) as file:
    for line in file:
        N, subblock_height, subblock_width, symbol_set = get_puzzle_information(line.strip())
        neighbors_dict = get_neighbors(line.strip())
        row, column, block = get_constraints(line.strip())
        solution = final_sudoku_solver(forward_looking([i if i != "." else "".join(str(j) for j in symbol_set) for i in line.strip()], neighbors_dict), neighbors_dict, row, column, block)
        if solution is not None:
            print("".join(solution))
            #display_puzzle("".join(solution), N, subblock_width, subblock_height)
            #print()
        else:
            print("No solution")